...
size_t USART_WriteData(const void *data, size_t dataSize){
	...
	for( ; i<dataSize; i++)
	{
		RingBuffer_PutChar(&USART_RingBuffer_Tx, (((char*)data)[i]));
	}
	__USART_ENABLE_IT(&UartHandle, USART_IT_TXE);
	return i;
}
...
size_t USART_ReadData(char *data, size_t maxSize){
	...
	for( ; i<maxSize; i++)
	{
		if(!USART_GetChar(&data[i])) return i;
	}
	return i;
}
...
void USARTx_IRQHandler(void){
    ...
    if (__HAL_USART_GET_IT_SOURCE(&UartHandle, USART_IT_TXE)) {
	    // the TXE interrupt is enabled
		  char c;
			if(RingBuffer_GetChar(&USART_RingBuffer_Tx, &c)) USARTx->DR = c;
			else __USART_DISABLE_IT(&UartHandle, USART_IT_TXE);
	  }
  }
}
